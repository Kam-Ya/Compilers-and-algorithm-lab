...
public class TestDriver {
    ...
    static <T extends Comparable<T>> T[][][] makeData(String[] types, int[] sizes) {
        T[][][] data = new Comparable[types.length][][];
    ...
    static <T extends Comparable<T>> void checkSort(T[][] arrays) {
    ...
    static <T extends Comparable<T>> void testSorts() {
        for (int sort = 0; sort < sorts.length; sort++) {
            T[][][] data = makeData(types, sizes);
            for (int type = 0; type < types.length; type++) {
